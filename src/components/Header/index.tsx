...
import { useSidebar } from '@/context/SidebarToggleStateProvider';

const Header = () => {
  const { sidebarOpen, setSidebarOpen } = useSidebar();
  return (
    <header className="sticky top-0 z-999 flex w-full border-b border-stroke bg-white dark:border-stroke-dark dark:bg-gray-dark">
      <div className="flex grow items-center justify-between px-4 py-5 shadow-2 md:px-5 2xl:px-10">
          ...
          >
            <span className="relative block size-5.5 cursor-pointer">
              ...
              <span className="absolute right-0 size-full rotate-45">
                ...
                ></span>
                <span
                  className={`absolute left-0 top-2.5 block h-0.5 w-full rounded-sm bg-dark delay-[400] duration-200 ease-in-out dark:bg-white ${
