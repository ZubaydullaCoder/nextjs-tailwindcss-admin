    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative hidden sm:block">
      <li>
        <Link
          ...
          href="#"
          className="relative flex size-12 items-center justify-center rounded-full border border-stroke bg-gray-2 text-dark hover:text-primary dark:border-dark-4 dark:bg-dark-3 dark:text-white dark:hover:text-white"
        >
          <span className="relative">
            ...
            <span
              className={`absolute -top-0.5 right-0 z-1 size-2.5 rounded-full border-2 border-gray-2 bg-red-light dark:border-dark-3 ${
                !notifying ? 'hidden' : 'inline'
              }`}
            >
              <span className="absolute -z-1 inline-flex size-full animate-ping rounded-full bg-red-light opacity-75"></span>
            </span>
          </span>
        </Link>
              ...
              {notificationList.map((item, index) => (
                <li key={index}>
                  <Link
                    ...
                    href="#"
                  >
                    <span className="block size-14 rounded-full">
                      <Image
                        width={112}
                        height={112}
                        src={item.image}
